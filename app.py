...
import concurrent.futures,hashlib,hmac,json,math,os,re,tempfile,threading,time
...
@app.get("/owner-link/<nonce>")
@rate_limit(5,300)
def owner_link(nonce):
    """Consume a one-use bootstrap link and establish the private owner cookie."""
    expected_hash=os.getenv("MOSQUITO_MAGIC_LINK_HASH","").strip().lower()
    dashboard_token=os.getenv("DASHBOARD_TOKEN","")
    supplied_hash=hashlib.sha256(nonce.encode("utf-8")).hexdigest()
    if not expected_hash or not dashboard_token or not hmac.compare_digest(supplied_hash,expected_hash):
        return error("Owner link is invalid",403)
    marker=Path(os.getenv("MOSQUITO_MAGIC_LINK_MARKER",f"/data/mosquito-magic-{expected_hash[:16]}.used"))
    marker.parent.mkdir(parents=True,exist_ok=True)
    try:
        fd=os.open(marker,os.O_CREAT|os.O_EXCL|os.O_WRONLY,0o600)
        with os.fdopen(fd,"w") as handle:handle.write(now())
    except FileExistsError:
        return error("Owner link has already been used",410)
    response=make_response(redirect("/"))
    response.set_cookie("mosquito_access",dashboard_token,httponly=True,secure=request.is_secure,samesite="Strict",max_age=2592000)
